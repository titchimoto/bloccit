...
RSpec.describe SessionsController, type: :controller do
  let (:my_user) { User.create!(name: "Theo Deane", email: "theo@meowmeow.com", password: "meowmeow") }
  ...
  describe "POST session" do
    ...
    it "does not add user to session due to missing password" do
      post :create, params: { session: { email: my_user.email } }
      expect(session[:user_id]).to be_nil
    end

    it "flashes error with bad email" do
